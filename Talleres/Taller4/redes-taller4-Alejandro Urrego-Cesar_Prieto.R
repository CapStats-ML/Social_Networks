...
p1 <- text_Caso3 %>%
  ...
  ggplot(aes(x = word, y = n, fill = n)) +  # Usar 'n' para definir el degradado
  theme_light() + 
...
text_Caso3 %>%
  count(word, sort = TRUE) %>%
  with(wordcloud(words = word, scale=c(4,1), freq = n, max.words = 100, colors= Col ))
...
xtable(head(frecuencia %>%
          arrange(desc(`CASO 8`)), 10), digits = 5, 
...
Caso3_Bi %>%
  ...
  rename(weight = n) -> Caso8_Bi_counts  # importante para la conformacion de la red!
...
layout2 <- layout_with_graphopt(g)

# Dibujar el gráfico con el layout Kamada-Kawai
plot(g, layout = layout, vertex.color = 1, vertex.frame.color = 1, 
